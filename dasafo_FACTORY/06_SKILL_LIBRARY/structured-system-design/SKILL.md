...
- **Gate Enforcement:** Physical transition to Phase (N+1) is BLOCKED if any TEA loop in Phase (N) returns `FAIL` or `REVISE`.
- **Traceability:** Every SSA decision MUST be documented in `PRP_CONTRACT.json` or `ADR.md` (physical persistence required).

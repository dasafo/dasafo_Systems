...
Solidified: Added 'finalize_blueprint' for automated 4-layer mapping (v4.0-MCP).
"""
...
def update_index(adr_dir: Path):
    ...
    md += "\n\n*Generated by Architecture Decision Records Engine (v4.0-MCP)*\n"
...
def run(skill_input: SkillInput) -> SkillOutput:
    ...
    try:
        ...
        if action == "init":
            ...
            # Escaneo DAST de las ADRs aceptadas
            files = sorted(list(adr_dir.glob("*.md")))
            adr_summaries = ""
            for f in files:
                if f.name not in ["README.md", "template.md"]:
                    adr_summaries += f"- **{f.name}**: Sincronizado con el diseño industrial.\n"

            blueprint_content = f"""# 🏗️ SYSTEM BLUEPRINT | {project_path.name}
> **Standard:** v4.0-MCP Industrial Core (SDD)
> **Status:** SOLIDIFIED

## 📐 1. Architecture Overview (4 Layers Mandate)
- **Domain:** Lógica pura y entidades de negocio (independiente de frameworks).
- **Application:** Casos de uso y orquestación de servicios.
- **Infrastructure:** Adaptadores, DB (PostgreSQL/Neo4j/Supabase) y APIs.
- **UI:** Interfaz de usuario (Next.js/Framer Motion) con componentes mudos.

## 📜 2. Consolidated Decisions (ADR Summary)
{adr_summaries}

---
*Generated by ARCHITECT Blueprinting Mandate (v4.0-MCP)*
"""
            ...
            return SkillOutput.success(
                ...
                skill=skill,
                result={
                    "industrial_status": "SOLIDIFIED - BLUEPRINT CREATED",
                    "path": str(blueprint_file),
                    "compliance_report": {
                        "layers_mapped": True, 
                        "execution_duration_seconds": round(time.time() - start_time, 4)
                    },
                    "summary": ["Technical Blueprint generated physically in DOCS/ARCH/."]
                },
                artifacts=[str(blueprint_file)],
                correlation_id=cid
            )
             ...
             adr_md += f"## 📖 Context\n\n{context}\n\n## ✅ Decision\n\n{decision}\n\n## ⚖️ Consequences\n\n{consequences}\n\n"
             adr_md += "---\n*ADR Recorded for dasafo_FACTORY v4.0-MCP*\n"

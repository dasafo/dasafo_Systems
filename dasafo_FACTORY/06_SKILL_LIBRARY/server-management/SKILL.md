...
- **Physical Access:** Operations MUST be verified via direct command output (e.g., `docker ps`, `systemctl`). Hallucinating a "NOMINAL" status while processes are failing is a SEVERITY-1 breach.
- **Resource Audit:** Scaling decisions MUST be backed by physical resource snapshots saved in `LOGS/infra/`.

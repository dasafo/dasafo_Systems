...
- **Atomic Persistence:** **v3.4.0-S MANDATE:** You must ensure the atomic movement of your tasks to the `03_COMPLETED` state using the `registry-manager` to maintain DAST synchronization.
...
- **Network Resilience (v3.4.3):** Before executing a deployment, verify the availability of the mapped ports on the Host. If a port is occupied, attempt to free it (if permissions allow) or report the conflict with the exact PID of the blocking process.
